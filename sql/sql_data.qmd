...
* Then open the `s2_setup.R` script by from the bottom-right hand files pane
...
+ largely just a matter of chaining together multiple sets of values
...
```{sql connection=src_memdb()$con}
ALTER TABLE clients ADD length INT;
```
...
ALTER TABLE clients ADD length INT; -- adding a column
