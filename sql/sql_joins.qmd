...
* Once you've logged-in to [posit.cloud](https://posit.cloud/), please select `New Project from Git Repository` </br>![New Rstudio project](src/images/sql_new_from_repo.png)
...
* Then open the `s4_setup.R` script by from the bottom-right hand files pane
* Click the `Source` button at the top right-hand corner of the source pane ![Source button](src/images/sql_source_script.png)
* Then open the `s4_joins.sql` script from the files pane, and `Ctrl` + `Shift` + `Enter` twice to preview your data for this session
...
read_rds("https://nes-dew.github.io/KIND-training/sql/data/prescribing_data.rds") |>
  list2env(.GlobalEnv)
...
SELECT GPPracticeName, PaidDateMonth, PaidQuantity, bnf_presentation 
  ...
  JOIN bnf_data AS bnf
  ON ab.BNFItemCode = bnf.bnf_presentation_code
  JOIN gp_practices AS gp ON ab.GPPractice = gp.PracticeCode -- multiple joins  
```
