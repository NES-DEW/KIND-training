...
```{r}
paste(">", paste(ggc_full$ggc_names[str_which(ggc_full$ggc_names, "\\.|\\d")], collapse = "  \n")) |>
  cat()

ggc_start <- paste(ggc_full$ggc_names[str_which(ggc_full$ggc_names, "\\.|\\d")], collapse = "\\n")
...
For the final few minutes of the session, we'll do an exercise in Excel or R depending on your preference. The task is simple: please accurately classify a small dataset into NHS GG&C/non-NHS GG&C. I believe there are 10 responses from GGC, and 13 not from GGC in the sample data.
...
This requires M365 Excel - either [the web version](https://excel.cloud.microsoft/) or the desktop version are fine. You can either [download the sample workbook](src/excel_regex.xlsx), or create a new workbook, and then copy and paste the following names into Excel:
...
Please [download the rds file](src/r_regex.rds) and import into R. While there are several ways of doing that, but a base-R solution would be something like the following:

```{r}
#| eval: false
#| echo: true

utils::download.file("https://nes-dew.github.io/KIND-training/skills/src/r_regex.rds", destfile = "r_regex.rds")
...
A neater alternative is to use the `stringr` package:
