...
Regular expressions are implemented in many coding languages, and in the more recent versions of Excel. For this session though we'll use a free-to-access regex writing tool: [regex101](regex101.com):

![Regex101.com interface](src/images/clipboard-3475536804.png)

Do note that [regex101](regex101.com) isn't suitable for any real data at all, and should only be used with synthetic examples. But it's a great training tool, not least because it gives explanations of how your regular expression is working.
...
Now paste them into the `Test String` area of regex101.com.
...
Try adding the regular expression `GG&C` to the `Regular expression` area of [regex101](regex101.com), and seeing what happens:
...
If we put some letters into those square brackets, our regex will match any of one letters. So this regex matches a `t`, and then either an `e`, and `h`, or an `o`.
...
If you need more precision about what a regex should select beyond the whole word/line methods we've been looking at, try groups. You formulate a group with round brackets - `()`. As an introduction, try the regex `NHS (GG&C)`:

![Groups example to match GG&C only when it follows NHS](src/images/groups_nhs.png)

That should select only `GG&C` when it follows NHS with a space. We could tweak that to also pick up the final string in our test pool with a bit of quantification:
...
This requires M365 Excel - either the web version or the desktop version are fine. Create a new workbook, and then copy and paste the following names into Excel:
