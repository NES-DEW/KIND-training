...
* now create a new SQL script (again, from the `File` menu). That new script will contain some pre-populated lines of code, as follows: </br>![Pre-populated code in new SQL script](src/images/new_sql.png)
* now replace that pre-existing first line with the following: `-- !preview conn=src_memdb()$con`
