...
3.  Pick `From Web` and supply [the URL](https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2023/2023-07-25/scurvy.csv) **https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2023/2023-07-25/scurvy.csv**
...
There are some limitations for most users about loading data from external sources at present. We'll demonstrate the usual way of loading data during the session, which is very similar to that used in Excel desktop. However, that appears at time of writing (July 2026) to be somewhat erratic, and possibly subject to different limitations in different organisations. The following is an additional fall-back way of creating and loading some data which might be helpful if the demonstrated method doesn't work for you:
...
4.  Then load that table into PowerQuery `Data` \> `Get & Transform Data` \> `From Table/Range` ![Loading data into PowerQuery](images/clipboard-2602479521.png)
:::
...
- use the following formula to create a joined column: `= [treatment] & " " & [dosing_regimen_for_scurvy]`: ![Add custom column](images/clipboard-799687413.png)
- here, the words in square brackets refer to the column names
...
![Close and load button](../src/images/clipboard-2525392040.png)
...
Can you find the number of responses to each question for your local NHS board? You should be able to re-work this data to give something like: ![Board-specific data extract](../src/images/clipboard-3334683078.png)
