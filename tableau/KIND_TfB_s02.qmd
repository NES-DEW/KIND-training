...
4.  drag and drop (or use the upload button) to add the `s02_weekend_data.xlsx` file
...
2.  add `s02_gp_data.xlsx` to your data and start a new worksheet
3.  drag the `Latitude` and `Longutude` fields to the column/row shelf. Avoid the `Latitude (generated)` and `Longitude (generated)` fields - they won't work properly
