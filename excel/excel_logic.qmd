...
+ we'll also see later uses of `*` and `+` to stand-in for `AND` and `OR` - those are just doing arithmetic on 0/1 values from logical functions

## IFS

-   `IFS` allows you to check for the presence of several conditions fairly concisely
- But `IFS` formulas are ghastly: would 100% recommend using the [Function arguments interface](https://support.microsoft.com/en-US/Excel/function-arguments) to build your formulas, rather than hand-writing them in the formula bar
