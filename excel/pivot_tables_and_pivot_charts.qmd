...
1. Press `Alt`, `N`, `V`, `T` (or from the ribbon `Insert >> PivotTable`), and click **OK** to insert a pivot table </br>![Insert pivot table](images/pt.png){height="150"}
1. Switch to the new worksheet containing your new (blank) pivot table </br>![New pivot table](images/pt_cols.png)
:::

## Adding data to a pivot table

+ next, we need to tell the pivot table which data we want to summarise
+ we'll use the PivotTable Fields interface to control this</br>![Pivot table fields](images/pt_cols_p.png)
...
![New Pivot table](images/pt_new.png)
...
1. We can also split each year into its own worksheet: find the option in the PivotTable Analyze section of the ribbon - or via `Alt`, `J`, `T`, `T`, `P` </br>![Pivot table pages](images/05_jttp.png)
...
2. From the ribbon, explore some of the pre-packed conditional formats </br>![Conditional formatting](images/05_cf.png)
:::
...
1. Bring up the conditional formatting rules manager using `ALT`, `H`, `L`, `R` </br>![Conditional formatting rules](images/05_cfrm.png)
:::
...
![Row sum formula](images/05_rowmax.png)
