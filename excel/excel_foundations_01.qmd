...
::: {.callout-tip title="Task"}
1. look at the values in the first 15 rows of the **date** column </br>![](images/1900.png)
...
1. we can format these into something human-readable. Please select all the cells in that date column and change the number format to **Short date** </br>![](images/format.png) 
		1. or use `Alt`, `H`, `N`, `S`
		1. or `Ctrl` + `1` and select from the dropdown
:::

+ cells are arranged in rows and columns </br> ![Rows and columns](images/cols.png)
...
3. then select **Format as Table** from the ribbon (or `Ctrl` + `T`) </br>![](images/table.png)
:::

+ you can also convert a table back into a range using the `Convert to Range` tool </br>![](images/01_converttorange.png)
...
::: {.callout-tip title="Task"}
1. using A1 reference, can you populate the first three rows of the summary table? </br>![](images/ref_task.png)
    a. Start date
    b. End date
...
::: {.callout-tip title="Task"}
1. the data accidentally includes the last day of the previous year</br>![](images/01_last_yr.png)
2. please remove that first row of data (click the row number, then `Ctrl` + `-`), and see how the references change
:::

![Relative references](images/refs.png)
...
+ this is done using the name box </br> ![The name box](images/01_name_box.png)
