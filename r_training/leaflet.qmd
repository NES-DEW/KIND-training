...
[^1]: If, like me, you struggle to remember which one is which, longitude is the East-West one. Earth is slightly wider than it is tall - an oblate spheroid, famously - so the longitude is slightly longer than the other one. Lots of interesting [history about longitude too](https://en.wikipedia.org/wiki/History_of_longitude) - it's probably the most-studied bit of history of science of all.
...
Using [this data set](data/locs.rds), we can build a map of Scotland's hospitals:
...
These are possible if you have a TopoJSON or geoJSON shape file of the boundaries you'd like to fill. [Fuller tutorial](https://rstudio.github.io/leaflet/articles/json.html), and some demo code based on the [Local Authority boundaries](https://data.spatialhub.scot/dataset/local_authority_boundaries-is):
...
![Filled map example](images/clipboard-3267343086.png)
