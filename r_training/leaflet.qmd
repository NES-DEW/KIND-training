...
`addMarkers()` creates markers to correspond with our data. Usefully, it'll interpret columns named lat(itude) and long(itude) as containing degrees of lat and long, so we've got a map of *something* here. But what's not exactly clear. Let's add an additional layer:
...
Bonus: https://en.wikipedia.org/wiki/History_of_longitude
