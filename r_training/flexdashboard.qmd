...
- you don't need any of this sample data to complete the session, but if you'd like to replicate the demonstrations, we're using two datasets:
...
- ![](images/clipboard-1313687372.png) will show you the preview in a browser
:::
...
We're basically designing columns here, and letting the `###` section dividers create rows for us. It'd also be possible to design the other way: rows first, then columns.
...
:::
 ...
 There's lots to explore in bslib, but probably the neatest way is to play with their built-in demo by running `bslib::bs_theme_preview()`
...
+ try changing the main theme (maybe to `cyborg` or `morph`) and 
:::
