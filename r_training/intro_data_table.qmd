...
```{r}
#| include: false
options(max.print=25) # otherwise endless df previews
```
...
There's [some sample data](https://github.com/arunsrinivasan/flights/wiki/NYCflights14/flights14.csv) that you might like to download and place in your project.
...
As a reminder, there are several techniques for manipulating that data frame. For example, we can subset the rows/cols of `mtcars` to discard parts of the data:

```{r}
mtcars[1] # column by index
mtcars["mpg"] # column by name
mtcars[1,] # row by index
mtcars[1,][1] # value by row and column index
```

There are also several ways of extracting parts of that data as a vector:

```{r}
mtcars[,1] # column by index
mtcars[["mpg"]] # column by name
mtcars$mpg # alternative, sweetened, way of column by name
mtcars[1,] |> 
  unlist() # usual way of taking a row as a vector
mtcars[3,7] # single value by row and column index
```

Much of the `data.table` syntax we'll look at in this session has a passing resemblance to these base R ways of working, but with several important differences. It's therefore useful to (re)familiarise yourself with these basics before moving on to the new material. 
...
### data.table
```{r}
# by design, dropping un-used columns:
dt[,.(dist_km = distance * 1.6, carrier, origin, dest)]

# or you can also effectively specify the columns to be retained as grouping variables
dt[,.(dist_km = distance*1.6),.(carrier,origin, dest)]

# or the := operator can modify a column in place. This allows you to change a single column in a data.table without having to re-include all the other columns. Note this permanently changes the object:
